...
|---|---|---|
| C01 | `app/trial_config.py`, `app/database.py`, `app/main.py`, `render.yaml`, `render.staging.yaml`, `requirements.txt`, `tests/production_infrastructure.py` | Provider-URL regression selects Psycopg 3; on 2026-07-23 the exact local candidate booted production-shaped configuration against disposable PostgreSQL 16 and Redis 7.4, enforced exact head, and returned readiness `200`; repeat in exact-candidate CI |
...
| M12 | One exact local redesign/edge-case/docs commit; blueprint auto-deploy is off | Owner-approved commit contains only the manifest paths; push and exact-candidate CI remain external |
...
| M15 | `.github/workflows/ci.yml` with least privilege, source/readiness checks, compile/tests, direct/resolved scans, disposable PostgreSQL migration/concurrency, Redis shared-limit/proxy, and production-readiness jobs | Workflow passed for committed base `8c775556`; local candidate source and clean-tree checks pass, while exact-candidate CI/staging remain absent |
